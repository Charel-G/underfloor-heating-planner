...
The solver library (glpk.js) is loaded from jsDelivr at runtime, so an
internet connection is required when opening the page unless you cache the
assets locally.
...
- An optional ILP solver runs in a Web Worker to globally optimize
  all circuits. The GLPK library is loaded from jsDelivr and fetches its `glpk.wasm` file automatically via `locateFile`. If the worker fails to load
  (for example when offline) or solving takes longer than two seconds, the planner automatically falls back to the heuristic A* router.
...
You can simply open `index.html` in your browser, but running a small local
server avoids any cross‑origin issues and ensures the GLPK WebAssembly module
loads correctly from the CDN. Use Python for example:
